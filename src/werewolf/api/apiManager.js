...
export class ApiManager {
  ...
  async getResponse(prompt, systemInstruction = null) {
    ...
    // 如果當前提供商沒有 API Key，嘗試使用另一個
    if (!this.apiKeys[provider]) {
      if (provider === 'gemini' && this.apiKeys.openai) {
        provider = 'openai';
        console.log('Gemini API Key 未設定，自動切換至 OpenAI');
      } else if (provider === 'openai' && this.apiKeys.gemini) {
        provider = 'gemini';
        console.log('OpenAI API Key 未設定，自動切換至 Gemini');
      }
    }
