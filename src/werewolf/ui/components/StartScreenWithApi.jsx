...
function StartScreen({ onStartGame, defaultPlayerCount = 8, defaultPlayerName = '', defaultUseAI = false, defaultGeminiKey = '' }) {
  ...
  // 元件載入時，嘗試從 localStorage 讀取先前儲存的設定
  useEffect(() => {
    try {
      const savedSettings = localStorage.getItem('werewolfGameSettings');
      ...
      if (savedSettings) {
        ...
        if (settings.useAI !== undefined) setUseAI(settings.useAI);
        if (settings.useAI && settings.geminiKey) setGeminiKey(settings.geminiKey);
      }
    } catch (error) {
      console.error('讀取儲存設定失敗:', error);
    }
  }, []);
  ...
  const handleSaveSettings = (e) => {
    ...
    try {
      ...
      // 如果啟用 AI，同時也儲存 API Key 到 WerewolfApi
      if (useAI && geminiKey && window.WerewolfApi) {
        window.WerewolfApi.saveApiKeys(geminiKey);
      }
              ...
              <div className="form-group">
                ...
                <div className="api-input-container">
                  ...
                  <button 
                    ...
                    disabled={apiTestStatus.testing || !geminiKey.trim()}
                  >
                    {apiTestStatus.testing ? '測試中...' : '測試 API'}
                  </button>
                </div>
